...
let handler = async (m, { conn, text }) => {
  ...
  if(!text && !m.quoted) return conn.reply(m.chat, `*『 RESET USER 』*\n\nTag user, tulis nomor, atau balas member yang ingin di RESET`, m)
  //let exists = await conn.isOnWhatsApp(number)
  // if (exists) return conn.reply(m.chat, `*Nomor target tidak terdaftar di WhatsApp*`, m)
  if(isNaN(number)) return conn.reply(m.chat, `*『 RESET USER 』*\n\nNomor yang kamu masukkan tidak valid !`, m)
  if(number.length > 15) return conn.reply(m.chat, `*『 RESET USER 』*\n\nNomor yang kamu masukkan tidak valid !`, m)
  try {
		if(text) {
